...
export default {
  ...
  methods: {
    ...
    async getLandingPageData() {
      try {
        let response = await $fetch(`http://bubbletea-werl.de:8080/auth/team`, {
          method: 'GET',
        });
        ...
        console.error("Fehler beim Abrufen der Team-Daten:", e);
      }
    },
...
// Methode zum Rückformatieren von HTML in Text für die Textareas
    ...
    async submitChanges() {
      ...
      try {
        ...
        if (response) {
          console.log("Team erfolgreich aktualisiert:", response);
          this.snackbarText = "Änderungen erfolgreich gespeichert";
          this.snackbarColor = "success";
        }
        this.dialog = false;
      } catch (e) {
        console.error("Fehler beim Aktualisieren der Team-Seite:", e);
