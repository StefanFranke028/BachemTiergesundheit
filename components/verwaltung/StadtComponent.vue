  ...
  methods: {
    async fetchDamen() {
      try {
        ...
        const response = await $fetch('http://5.45.97.75/auth/dame');
    ...
    async getStaedte() {
      ...
      try {
        let response = await $fetch(`http://5.45.97.75/auth/stadt`, {
          method: 'GET',
        });
...
// Methode zum Rückformatieren von HTML in Text für die Textareas
    ...
    async submitChanges() {
      ...
      try {
        ...
        if (this.editStadt) {
          response = await $fetch(`http://5.45.97.75/auth/stadt/${this.editStadt.id}`, {
            method: 'PUT',
            body: data
          });
          ...
          response = await $fetch(`http://5.45.97.75/auth/stadt`, {
            method: 'POST',
            body: data
          });
    ...
    async confirmDelete() {
      ...
      try {
        let response = await $fetch(`http://5.45.97.75/auth/stadt/${this.stadtToDelete.id}`, {
          method: 'DELETE',
        });
