              ...
              >
                <v-row class="justify-center mt-3">
                  <v-col class="d-flex justify-center" cols="5">
                    <v-text-field v-model="url" label="Name" variant="outlined" />
                  </v-col>
                  <v-col class="d-flex justify-center" cols="5">
  ...
  methods: {
    ...
    async create() {
      if (process.client) {
        ...
        try {
          ...
          await $fetch('https://tier-gesundheitszentrum.com:8080/auth/seminare', {
            method: 'POST',
            body,
            headers: {
    ...
    async deleteMethod(bericht) {
      if (process.client) {
        try {
          const token = localStorage.getItem('token')
          await $fetch(`https://tier-gesundheitszentrum.com:8080/auth/seminare/${bericht.id}`, {
            method: 'DELETE',
            headers: {
              Authorization: `Bearer ${token}`
            }
    ...
    async get() {
      if (process.client) {
        try {
          const token = localStorage.getItem('token')
          const response = await $fetch('https://tier-gesundheitszentrum.com:8080/auth/seminare', {
            baseURL: '',
            headers: {
              Authorization: `Bearer ${token}`
            }
