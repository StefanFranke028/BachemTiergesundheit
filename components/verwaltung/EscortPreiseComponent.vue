...
export default {
  ...
  methods: {
    async getEscortPreise() {
      try {
        let response = await $fetch(`http://5.45.97.75:8080/auth/escortpreise`, {
          method: 'GET',
        });
...
// Methode zum Rückformatieren von HTML in Text für die Textareas
    ...
    async submitChanges() {
      ...
      try {
        ...
        if (this.editEscortPreis) {
          ...
          response = await $fetch(`http://5.45.97.75:8080/auth/escortpreise/${this.editEscortPreis.id}`, {
            method: 'PUT',
            body: data
          });
          ...
          response = await $fetch(`http://5.45.97.75:8080/auth/escortpreise`, {
            method: 'POST',
            body: data
          });
    ...
    async confirmDelete() {
      ...
      try {
        let response = await $fetch(`http://5.45.97.75:8080/auth/escortpreise/${this.escortPreiseToDelete.id}`, {
          method: 'DELETE',
        });
