...
// Methode zum Rückformatieren von HTML in Text für die Textareas
    ...
    async getBlogEintraege() {
      this.loading = true;
      try {
        let response = await $fetch(`http://85.215.175.241:8080/auth/blog/entries`, {
          method: 'GET',
        });
    ...
    async confirmDelete() {
      ...
      try {
        let response = await $fetch(`http://85.215.175.241:8080/auth/blog/entries/${this.entryToDelete.id}`, {
          method: 'DELETE',
        });
    ...
    async submitChanges() {
      ...
      try {
        ...
        if (this.editedEntry) {
          ...
          response = await $fetch(`http://85.215.175.241:8080/auth/blog/entries/${this.editedEntry.id}`, {
            method: 'PUT',
            body: data
          });
          ...
          response = await $fetch(`http://85.215.175.241:8080/auth/blog/entries`, {
            method: 'POST',
            body: data
          });
